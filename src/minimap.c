...
/*   Updated: 2025/09/12 15:21:20 by nass             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_minimap(t_data *data)
{
	...
	while (mp.y < data->map_height)
	{
		...
		while (++mp.x < data->map_width)
		{
			...
			draw_square(data, mp.draw_x, mp.draw_y, mp.tile_size, mp.color);
		}
		mp.y++;
	}
	mp.player_draw_x = mp.offset_x + data->player_x * mp.tile_size;
	mp.player_draw_y = mp.offset_y + data->player_y * mp.tile_size;
	draw_square(data, mp.player_draw_x, mp.player_draw_y, mp.tile_size, RED);
}
